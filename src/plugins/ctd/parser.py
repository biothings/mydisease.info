...
def process_pathway(file_path_pathway):
    ...
    df_disease_pathway = pd.read_csv(file_path_pathway, sep=',', comment='#', compression='gzip',
                                     names=['DiseaseName', 'DiseaseID', 'pathway_name',
                                            'pathway_id', 'inference_gene_symbol'],
                                     dtype=str)
    d = {}
    ...
    for grp, subdf in df_disease_pathway.groupby(['DiseaseID', 'pathway_id']):
        ...
        record_dict[tempPathwayID[0].lower(
        ) + '_pathway_id'] = tempPathwayID[1]
    ...
    for chunk in pd.read_csv(
        ...
        if not temp_chunk.empty:
            ...
            for field_id in ['omim_id', 'pubmed']:
                temp_chunk[field_id] = temp_chunk[field_id].apply(
            ...
            for did, subdf in temp_chunk.groupby('DiseaseID'):
                ...
                chemical_related = [{k: v for k, v in record.items(
                ) if k not in {'DiseaseName', 'DiseaseID'}} for record in records]
...
def load_data(data_folder):
    file_path_disease_pathway = os.path.join(
        data_folder, 'CTD_diseases_pathways.csv.gz')
    file_path_disease_chemical = os.path.join(
        data_folder, 'CTD_chemicals_diseases.csv.gz')
